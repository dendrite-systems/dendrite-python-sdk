...
class ExtractionMixin(DendritePageProtocol):
    ...
    def extract(
        ...
        if use_cache:
            cache_available = self.check_if_extract_cache_available(prompt, json_schema)
            if cache_available:
                logger.info("Cache available, attempting to use cached extraction")
                result = self.attempt_extraction_with_backoff(
                    prompt,
                    json_schema,
                    only_use_cache=True,
    ...
    def check_if_extract_cache_available(
        ...
        cache_response: CacheExtractResponse = (
            self._get_browser_api_client().check_extract_cache(dto)
        )
