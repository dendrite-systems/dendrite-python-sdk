...
from dendrite_server_merge.dto.AskPageDTO import AskPageDTO
from dendrite_server_merge.responses.AskPageResponse import AskPageResponse
...
async def ask_page_action(
    ...
    while iteration < max_iterations:
        ...
        res = await async_claude_request(config, ask_page_dto.api_config)
        ...
        if "scroll_down" in data_dict:
            ...
            try:
                if ask_page_dto.return_schema:
                    validate(
                        instance=return_data, schema=ask_page_dto.return_schema
                    )
...
def generate_ask_page_prompt(ask_page_dto: AskPageDTO, image_segments: list, scrolled_to_segment_i: int = 0) -> list:
