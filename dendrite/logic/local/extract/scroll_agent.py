...
class ScrollAgent:
    def __init__(self, api_config, page_information: PageInformation):
    ...
    async def process_segment(self, messages: List[dict]) -> dict:
        config = {
            "messages": messages,
        ...
        if not isinstance(res.content[0], TextBlock):
            raise Exception("Response needs to be a text block")
...
Below is a screenshot of the current page, if it looks blank or empty it could still be loading. If this is the case, don't guess what elements to inspect, respond with is loading.""",
    ...
    def create_scroll_message(self, image: str) -> dict:
        return {
            "role": "user",
            "content": [
